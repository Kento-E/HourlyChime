...
object BluetoothHelper {
  ...
  /** ペアリング済みデバイスの一覧を返す。 権限がない場合は空リストを返す。 */
  ...
  /** 対象アドレスのいずれかのデバイスが現在接続中かどうかを返す。 権限がない場合は false を返す（フェイルクローズ）。 */
  ...
  /** BluetoothDevice が現在接続中かどうかを返す。 `isConnected()` は隠しAPIだが全APIレベルで安定して存在する。 */
  ...
  /** BluetoothDevice が現在接続中かどうかを返す（BluetoothEventReceiver 用）。 */
  fun isDeviceConnectedPublic(device: BluetoothDevice): Boolean {
    return try {
      val method = device.javaClass.getMethod("isConnected")
      method.invoke(device) as Boolean
    } catch (e: Exception) {
